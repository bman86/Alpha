...
class ManagersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create manager" do
    assert_difference('Manager.count') do
      post managers_url, params: { manager: { jobs: @manager.jobs, name: @manager.name, title: @manager.title } }
    end
  ...
  test "should update manager" do
    patch manager_url(@manager), params: { manager: { jobs: @manager.jobs, name: @manager.name, title: @manager.title } }
